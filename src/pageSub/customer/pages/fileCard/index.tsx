...
const FileCard: FC = () => {
  ...
  useEffect(() => {
    getList(1, true);
    eventCenterOn("fileCardEdit", () => {
      getList(1, true);
    });
    return () => {
      eventCenterOff("fileCardEdit");
    };
  }, []);
    ...
    <View className="flex-page" style={{ flex: 1, backgroundColor: "#f7f8f8" }}>
                ...
                {pageList.map((item: any, index) => {
                  return (
                    <View
                      ...
                      onClick={()=>{
                        navigateTo({ url: "/pages/customer/fileCard/edit/index", method:"navigateTo", params: { id: item.id } });
                      }}
                    >
                      <Cell
                        border={false}
                        ...
                        title={item.name}
                        label={`可建立数量：${item.min_num === item.max_num ? item.min_num : `${item.min_num}-${item.max_num}`}`}
                        labelStyle={{ color: "#666" }}
                        extraStyle={{ marginTop: addUnit(2), display: "flex", flexDirection: "row" }}
                        extra={
                          <React.Fragment>
                            {item.is_record ? <Tag plain={false} type="info">数据记录</Tag> : null}
                            {item.is_co_order ? <Tag plain={false} type="success" style={{ marginLeft: addUnit(8) }}>企业服务</Tag> : null}
                          </React.Fragment>
                        }
                      />
                      <Cell
      ...
      <BottomBar>
        <Button
          ...
          type="info"
          onClick={() => {
            Taro.navigateTo({ url: "/pages/customer/fileCard/edit/index" });
          }}
        >
          新建档案卡
        </Button>
