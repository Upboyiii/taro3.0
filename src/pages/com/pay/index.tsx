...
const Pay: FC = () => {
  ...
  const isDiscount = store.getState().storeConfig.storePay && store.getState().storeConfig.storePay.CASH_DISCOUNT == "1";
      ...
      {!err ? (
        <React.Fragment>
          <ScrollView scrollWithAnimation style={scrollViewStyle()} scrollY>
            ...
            {isDiscount ? (<View className="card-tips">如需优惠，需在支付提交前填写对应优惠金额，支付成功后即可享受优惠</View>) : null}
            <View className="card">
              <Cell
                title="需付金额"
                contentStyle={{ fontSize: addUnit(24), color: "#ff2340", fontWeight: "bold", lineHeight: addUnit(30) }}
        ...
        <Empty desc={`${err || "加载错误"}，请返回重试`} image="error" />
      )}
      {/* <Popup*/}
      {/*  show={showQRCodeImg}*/}
      {/*  title="请扫码付款"*/}
