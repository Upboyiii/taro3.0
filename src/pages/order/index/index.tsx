...
const Order: FC = () => {
  ...
  const [filter, setFilter] = useState(config.userOrder.filterInit);
  ...
  const getList = (page, refresh = false) => {
    ...
    orderApi.userOrder.page({ page: page, page_size: pageSize, ..._filter }).then(res => {
      if (!!res && res.code === 0) {
  ...
  return (
    <View className="flex-page" style={{ flex: 1, backgroundColor: "#f7f8f8" }}>
      <Navbar
        ...
        onClickRight={() => {
          navigateTo({ url: "/pages/order/filter/index", method: "navigateTo", params: { form: filter, refs: "userOrder" } });
        }}
      />
      <Tabs active={tabsActive} options={config.userOrder.stateList} onChange={handelTabs} bgColor="#fff" border scroll space={12} />
      <ScrollView
        err={err}
        refresh
                ...
                {pageList.map((item: any, index) => {
                  return (
                    <View
                      ...
                      onClick={() => {
                        navigateTo({ method: "navigateTo", url: "/pages/order/index/detail/index?id=" + item.id });
                      }}
                    >
                      <Cell
                        border={false}
                      ...
                      >
                        ...
                        <View>
                          <View style={{ fontSize: addUnit(10), color: "#999", textAlign: "right" }}>实付金额</View>
                      ...
                      >
                        <View>
                          ...
                          <View style={{ fontSize: addUnit(14), color: "#333", marginTop: addUnit(2) }}>
                            {findLabel(item.type, config.userOrder.typeList)}
                          </View>
                        </View>
                        {!!item.sales_state ? (
                          <View>
                            <View style={{ fontSize: addUnit(10), color: "#999", textAlign: "center" }}>售后状态</View>
                            <View style={{ fontSize: addUnit(14), color: "#333", textAlign: "center", marginTop: addUnit(2) }}>
                              {findLabel(item.sales_state, config.userOrder.salesList)}
                            </View>
                          </View>
                        ) : null}
                        <View>
                          ...
                          <Tag
                            ...
                            type={
                              item.state === 1 ? "primary" :
                                item.state === 3 || item.state === 4 ? "success" :
                                  item.state === 6 ? "warning" :
                                    item.state === 7 || item.state === 10 || item.state === 20 ? "info" : "default"
                            }
                            style={{ width: "60px", float: "right" }}
                          >
                            {findLabel(item.state, config.userOrder.stateList)}
                          </Tag>
                        }
                      />
                    </View>
