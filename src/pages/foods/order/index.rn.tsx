...
const FoodsOrder: FC = () => {
    ...
    if(!!_goodsInfo.goods_rule_sku){
      ...
      if(_goodsInfo?.goods_rule_sku.length > 0 && _goodsInfo.goods_rule_sku[0].children){
        if(_goodsInfo.goods_rule_sku[0].children.length > 0 && _goodsInfo.goods_rule_sku[0].children[0].sku_id){
          _checkSku = _goodsInfo.goods_rule_sku[0].children[0];
          _checkSku.pay_num = _checkSku.stocks.stock_num > 0 || _checkSku.stocks.stock_num === -999 ? 1 : 0;
        }
      }
    }
  ...
  const addCart = () =>{
    ...
    let obj = {
      ...
      price: (_checkSku.stocks.price + (_checkSku.attrs_price ? _checkSku.attrs_price : 0)) * _checkSku.pay_num,
  ...
  const addOrder = ()=>{
    ...
    if(_params.flow_id){
      const otherList = orderForm.otherShopList
        .map((item:any) => {
          if (item.checkNum > 0) {
        ...
        .filter(item => !!item);
      if(checkList.length <= 0 && otherList.length <= 0){
        setPopupShow({
          visable: false,
          title: "",
          type: 1
        });
        Taro.showToast({ title: "请选择菜品或者其他商品", icon: "none" });
        return;
      }
    }
    let list = [],order_list = [];
            ...
            {groupList.map((item,index)=>{
              return (
                ...
                >
                  ...
                  {index === 0 ? (<View  className={classnames(bem("aside-badge"))}></View>) : null}
                </View>
              );
            })}
          </ScrollView>
        ...
        {popupShow.type === 1 ? (
                    ...
                    <View className={classnames(bem("goods-info"))}>
                      <View className={classnames(bem("goods-price"))}>
                        ...
                        <View className={classnames(bem("goods-price__price"))}>
                          {formatPrice(((goodsDetail.checkSku?.stocks?.price || 0) + (goodsDetail.checkSku?.attrs_price || 0)) * (goodsDetail.checkSku?.pay_num || 1))}
                        </View>
                      </View>
          ...
          <KeyboardAwareScrollView>
            ...
            <View className="card">
              ...
              />
              {orderForm.otherShopList.length > 0 ? (
                <React.Fragment>
                      ...
                      <Cell
                        ...
                        extra={
                          <Stepper
                            value={item.checkNum}
                            theme="tinge"
                            min={_params?.flow_id ? 0 :  item.is_must ? 1 : 0}
                            round
                            plain
                            buttonPlusPrimary
                            onChange={(val)=>{
